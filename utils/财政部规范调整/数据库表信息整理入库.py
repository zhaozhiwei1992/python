...
def getFieldDataListFromJC():
    ...
    try:
        ...
        sql = (
            ...
            "FROM user_tab_cols t, user_col_comments t2, fasp_t_dictable t3, FASP_T_DICCOLUMN t4 WHERE t.table_name = t2.table_name (+) AND t.column_name = t2.column_name (+) AND t.table_name = t3.TABLECODE(+) AND t.table_name = t4.TABLECODE(+) AND t.COLUMN_NAME = t4.COLUMNCODE(+) AND t.TABLE_NAME LIKE 'PAY%'")
        cursor.execute(sql)
        # 获取字段描述信息
...
if __name__ == '__main__':
    ...
    saveDataToOracle("STANDARD_FIELD_V1_JC", tableColList)
